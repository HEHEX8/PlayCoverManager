...
├── lib/                           # ライブラリモジュール
│   ├── 00_core.sh                # ✅ コア機能（500行 - 完成）
│   ├── 01_mapping.sh             # ✅ マッピング管理（170行 - 完成）
│   ├── 02_volume.sh              # ✅ ボリューム操作（260行 - 完成）
│   ├── 03_storage.sh             # 🚧 ストレージ管理（734行 - 基本完成、移行関数未完）
│   ├── 04_app.sh                 # 🚧 アプリ管理（未実装）
...
### フェーズ2: 段階的移行 🚧
- [x] マッピング管理モジュール実装（01_mapping.sh - 170行）
- [x] ボリューム操作モジュール実装（02_volume.sh - 260行）
- [x] ストレージ管理モジュール実装 - 基本完成（03_storage.sh - 734行）
  - [x] ストレージ検出ロジック
  - [x] 容量計算関数
  - [x] フラグ管理
  - [x] UIとメニュー
  - [ ] 移行ヘルパー関数の完全実装（rsync操作）
- [ ] アプリ管理モジュール実装
- [ ] クリーンアップモジュール実装
- [ ] セットアップモジュール実装
- [ ] UIモジュール実装
...
- 初期モジュール構造作成
- コアモジュール完成（00_core.sh - 500行）
- マッピング管理モジュール完成（01_mapping.sh - 170行）
- ボリューム操作モジュール完成（02_volume.sh - 260行）
- ストレージ管理モジュール基本完成（03_storage.sh - 734行）
  - 検出ロジック、容量計算、フラグ管理完成
  - 移行ヘルパー関数は骨格のみ（次回完成予定）
- メインエントリーポイント作成

---

**最終更新**: 2025-10-28  
**ステータス**: 開発中（フェーズ2 - 3/8モジュール完成）
