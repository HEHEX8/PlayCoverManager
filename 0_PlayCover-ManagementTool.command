...
show_menu() {
    ...
    echo "  ${LIGHT_GREEN}6.${NC} リムーバブルドライブ自動マウント設定"
...
# Install disk monitoring LaunchAgent for automatic mounting
install_disk_monitor() {
    clear
...
# Check if PlayCover volume exists on any drive
check_playcover_drive() {
    diskutil list 2>/dev/null | grep -q "$PLAYCOVER_VOLUME_NAME"
}
...
mount_all_volumes() {
    ...
    while IFS=$'\t' read -r volume_name bundle_id display_name; do
        ...
        if mount | grep -q " on ${target_path} "; then
            log_message "INFO: '${volume_name}' は既にマウント済み"
            ((success_count++))
            continue
        fi
    ...
    log_message "INFO: マウント処理完了 (成功: ${success_count}, 失敗: ${fail_count})"
...
if check_playcover_drive; then
    log_message "INFO: PlayCoverボリュームを検知"
    mount_all_volumes
else
    log_message "INFO: PlayCoverボリュームが見つかりません（ドライブ未接続）"
fi
...
<dict>
    <key>Label</key>
    <string>com.playcover.diskmount</string>
...
# Uninstall disk monitoring LaunchAgent
uninstall_disk_monitor() {
    ...
    echo "  ${HOME}/Library/Logs/playcover-disk-monitor.log"
    wait_for_enter
}
...
show_auto_mount_menu() {
    while true; do
        ...
        echo "${CYAN}機能説明:${NC}"
        echo "  • PlayCoverボリュームを含むドライブ接続時に自動検知"
        echo "  • 登録されている全ボリュームを自動マウント"
        echo "  • /Volumes/ ディレクトリを監視（WatchPaths）"
        echo "  • 内蔵データがある場合は安全にスキップ"
        echo ""
        print_separator
        echo ""
