...
### 開発版: v5.0.0-alpha1（Phase 3完了）
- **ディレクトリ**: `playcover-manager/`
- **状態**: ✅ Phase 3完了（全モジュール実装・包括的検証済み）
...
4. **ストレージ切り替え（内蔵⇄外部）**
   ...
   - ストレージモードの自動検出（外部/内部/汚染/空）
   - 空のソースディレクトリの自動処理（v4.42.2+）
   - 差分同期による効率的な転送（内蔵→外部）
   - データ保持型の完全コピー（外部→内蔵）
   - 容量チェックと警告表示
   - フラグシステムによる意図的/意図しない内蔵データの識別

5. **🔥 超強力クリーンアップ（完全リセット）** ← **隠しオプション**
   - **メニューに表示されません**（誤操作防止）
   - **起動方法**: メニューで `X`、`x`、`RESET`、または `reset` を入力
   - すべてのボリュームをアンマウント
   - すべてのコンテナを完全削除
...
### 🔥 完全リセット（トラブル時）
1. `0_PlayCover-ManagementTool.command` をダブルクリック
2. メインメニューで **`X`、`x`、`RESET`、または `reset`** を入力（隠しオプション）
3. 削除対象の完全プレビューを確認
4. `yes` と入力
5. `DELETE ALL` と入力して実行
6. 完了後、ターミナルが自動的に閉じる
7. **再セットアップが必要**: 再度スクリプトを起動して初期セットアップから実行
...
├── playcover-manager/                   # ✅ モジュール化版（Phase 3完了 v5.0.0-alpha1）
│   ├── main.sh                         #    メインエントリーポイント（101行）
│   ├── lib/                            #    モジュールライブラリ
│   │   ├── 00_core.sh                 #    ✅ コア機能（458行、13関数）
│   │   ├── 01_mapping.sh              #    ✅ マッピング管理（172行、9関数）
│   │   ├── 02_volume.sh               #    ✅ ボリューム操作（505行、14関数）
│   │   ├── 03_storage.sh              #    ✅ ストレージ管理（1,207行、17関数）
│   │   ├── 04_app.sh                  #    ✅ アプリ管理（1,106行、25関数）
│   │   ├── 05_cleanup.sh              #    ✅ クリーンアップ（404行、3関数）
│   │   ├── 06_setup.sh                #    ✅ 初期セットアップ（471行、8関数）
│   │   └── 07_ui.sh                   #    ✅ UIとメニュー（900行、13関数）
│   ├── tests/                          #    テストディレクトリ
...
### v5.0.0-alpha1 (開発中) - Modular Architecture Phase 3 Complete
- 🏗️ **モジュール化構造の完成**: 5,380行を8つのモジュールに分割（5,624行、91関数）
  * ✅ **Phase 1完了**: 基本構造とコアモジュール
  * ✅ **Phase 2完了**: 全モジュール実装（8モジュール + main.sh）
  * ✅ **Phase 3完了**: 包括的検証（11段階すべてクリア）
...
- 📦 **モジュール構成**（全完成）:
  ...
  * 06_setup.sh - 初期セットアップ（471行、8関数）
  * 07_ui.sh - UIとメニュー（900行、13関数）
  * main.sh - エントリーポイント（101行、1関数）
- ✅ **検証完了**: 重複関数0件、未定義関数0件、未宣言変数0件
...
- 🔒 **超強力クリーンアップの隠しオプション化**: 誤操作防止のためメニューから非表示に
  ...
  * 隠しオプション: `X`、`x`、`RESET`、`reset` キーで実行可能
  * 意図的な操作のみ実行可能に
- 🧪 **ボリューム操作検証ツール追加**: 開発・デバッグ用の検証ツールを追加
  * `volume_operations_test_plan.md`: 10種類の詳細な検証パターン定義
  * `verify_volume_operations.sh`: 自動検証スクリプト
    - 空ソース検出の検証
    - データサイズ計算の精度確認
    - rsync 動作パターンの検証（--delete の有無）
    - ファイル権限処理の確認
    - ボリューム操作の基本動作確認
  * テストデータの自動生成/クリーンアップ機能
  * 検証結果のレポート機能

### v4.42.2 - Empty Source Directory Handling
- 🔧 **空ソースディレクトリ処理追加**: 0バイトのディレクトリを正しく処理
  * 内蔵→外部で内蔵が空: rsyncスキップ、空ディレクトリ削除、外部直接マウント
  * 外部→内蔵で外部が空: rsyncスキップ、空の内蔵ディレクトリ作成、外部アンマウント
  * 空文字列または0バイトの両方をチェック
- ✅ **データ損失防止**: rsync --delete による意図しない削除を回避
- ⚡ **処理効率化**: 不要なrsync実行を省略

### v4.42.1 - Fixed Capacity Check for Storage Switch
- 🐛 **容量チェック修正**: 内蔵→外部転送時の容量チェック対象を修正
  * 修正前: PlayCoverメインボリュームの容量をチェック（誤り）
  * 修正後: 転送先アプリボリュームの容量をチェック（正しい）
- ✅ **誤警告の解消**: 正しいボリュームで容量を判定

### v4.42.0 - Mapping File Location Change
- 📁 **マッピングファイル配置変更**: ホームディレクトリからスクリプトディレクトリへ
  * 変更前: `~/.playcover-volume-mapping.tsv`
  * 変更後: `${SCRIPT_DIR}/.playcover-volume-mapping.tsv`
- ✅ **可搬性向上**: スクリプトと同じ場所にデータを配置
- 🧹 **整理整頓**: ホームディレクトリの散らかりを解消

### v4.41.0 - Complete Removal of Auto-Mount Feature
- 🗑️ **自動マウント機能の完全削除**: 566行のコード削除
  * LaunchAgent/LaunchDaemon関連の全機能を削除
  * ディスクモニター機能を削除
  * メニューオプション6を削除
  * 選択範囲を (0-6) から (0-5) に変更
- ✅ **簡素化**: 実装が困難であった機能を削除し、メンテナンス性を向上

### v4.33.5 - Improved Wrong Mount Location Handling with Auto-Remount
