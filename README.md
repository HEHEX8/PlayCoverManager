...
6. IPA ファイル選択（GUI）
7. IPA 情報自動抽出（日本語名・バージョン含む）
8. インストール先ディスク自動選択
9. **アプリ専用ボリューム作成（確認プロンプト付き）**
   - アプリ名（日本語）
   - バージョン情報
   - Bundle ID
   - ボリューム名
10. データ競合処理
11. マッピングデータ登録
12. **PlayCover へインストール要求（アップデート確認付き）**
    - 既存アプリがある場合はバージョン比較
    - 上書き確認プロンプト
13. 完了後自動終了
...
- **ボリューム作成確認**
  ...
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    アプリ名: ゼンレスゾーンゼロ
    バージョン: 1.4.0
    Bundle ID: com.HoYoverse.Nap
    ボリューム名: ZenlessZoneZero
    作成先: disk5
...
- ✅ **ユーザー確認プロンプトの追加**
  - ボリューム作成前の確認プロンプト
  - アプリ情報（日本語名、バージョン、Bundle ID）の表示
  - 既存アプリ上書き時の詳細な確認画面
  - インストール済み vs 新規バージョンの比較表示
- ✅ **日本語アプリ名の抽出**
  - `ja.lproj/InfoPlist.strings` からの日本語名取得
  - 日本語名がない場合は英語名にフォールバック
  - ボリューム名は英語名ベースで作成（互換性維持）
- ✅ **バージョン情報の表示**
  - IPA 解析時にバージョン情報を抽出
  - 既存アプリのバージョンも表示
  - アップデート判断の材料を提供
