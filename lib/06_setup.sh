...
#######################################################
# fcp (Fast Copy) Check and Installation
#######################################################

check_fcp_installation() {
    print_header "高速コピーツール (fcp) の確認"
    
    if command -v fcp >/dev/null 2>&1; then
        local fcp_version=$(fcp --version 2>&1 | head -n 1 || echo "不明")
        print_success "fcp が存在します"
        print_info "${fcp_version}"
        echo ""
        return 0
    else
        print_warning "fcp が見つかりません"
        print_info "fcpはRust製の高速ファイルコピーツールです"
        print_info "標準のrsyncよりも20-30%高速で、並列処理により大幅な速度向上が期待できます"
        echo ""
        
        echo -n "${YELLOW}Homebrewでfcpをインストールしますか？ (y/N):${NC} "
        read install_choice
        echo ""
        ...
        if [[ "$install_choice" =~ ^[Yy]$ ]]; then
            ...
            print_info "スキップしました"
            print_warning "fcpなしでも動作しますが、標準のcpコマンドを使用します"
            echo ""
            return 1
        fi
    fi
...
install_fcp() {
    print_header "fcp のインストール"
    
    print_info "Homebrewでfcpをインストール中..."
    echo ""
    
    # Try to install via Homebrew
    if "$BREW_PATH" install fcp 2>&1; then
        echo ""
        print_success "fcp のインストールが完了しました"
        echo ""
        return 0
    else
        echo ""
        print_warning "Homebrewでのインストールに失敗しました"
        print_info "代わりにCargoでインストールを試みます..."
        echo ""
        
        # Check if cargo is available
        if command -v cargo >/dev/null 2>&1; then
            print_info "cargo install fcp を実行中..."
            echo ""
            
            if cargo install fcp 2>&1; then
                echo ""
                print_success "fcp のインストールが完了しました"
                ...
                if [[ -d "$cargo_bin" ]] && [[ ":$PATH:" != *":$cargo_bin:"* ]]; then
                    ...
                    print_info "PATHに~/.cargo/binを追加しました"
                fi
                ...
                echo ""
                print_error "Cargoでのインストールも失敗しました"
                print_info "標準のcpコマンドを使用します"
                echo ""
                return 1
            fi
        else
            print_warning "Cargoが見つかりません"
            print_info "Rustをインストールするには: https://rustup.rs"
            print_info "標準のcpコマンドを使用します"
            echo ""
            return 1
        fi
