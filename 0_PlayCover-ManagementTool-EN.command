...
# - PlayCover: Installed via Homebrew Cask
# - External Storage: USB/Thunderbolt/SSD (APFS formatted)
# 
# [Used Commands (macOS Standard)]
...
# - Homebrew (brew): Required for PlayCover installation
# - PlayCover.app: /Applications/PlayCover.app
# 
# [Important Notes]
# - Some operations require sudo privileges (diskutil, mount, etc.)
# - Selecting wrong external storage may cause data loss
# - Nuclear cleanup deletes all data (irreversible)
#
#######################################################
...
# Optimized for: Terminal background RGB(28,28,28) / #1C1C1C
# Human color perception considered: Reduced glare + Enhanced visibility
# ═══════════════════════════════════════════════════════════════════
...
readonly BLINK='\033[5m'             # Blink (not recommended)
...
readonly WHITE='\033[38;2;230;230;230m'      # Soft White #E6E6E6 (17.5:1) - Reduced glare
readonly LIGHT_GRAY='\033[38;2;180;180;180m' # Light Gray #B4B4B4 (9.8:1) - Standard text
readonly GRAY='\033[38;2;140;140;140m'       # Medium Gray #8C8C8C (5.8:1) - Supplementary info (enhanced visibility)
readonly DIM_GRAY='\033[38;2;110;110;110m'   # Dark Gray #6E6E6E (4.6:1) - Minimum contrast

# ─── Semantic Colors (Reduced saturation for eye comfort) ───
readonly RED='\033[38;2;255;120;120m'        # Soft Red #FF7878 (9.5:1) - Reduced glare
readonly GREEN='\033[38;2;120;220;120m'      # Soft Green #78DC78 (11.8:1) - Reduced saturation
readonly BLUE='\033[38;2;120;180;240m'       # Soft Blue #78B4F0 (10.2:1) - Gentle blue
readonly YELLOW='\033[38;2;230;220;100m'     # Soft Yellow #E6DC64 (14.5:1) - Reduced glare
readonly CYAN='\033[38;2;100;220;220m'       # Soft Cyan #64DCDC (12.2:1) - Reduced saturation
readonly MAGENTA='\033[38;2;220;120;220m'    # Soft Magenta #DC78DC (9.8:1) - Gentle purple

# ─── Extended Colors (Natural tones for extended use) ───
readonly ORANGE='\033[38;2;240;160;100m'     # Natural Orange #F0A064 (10.5:1)
readonly GOLD='\033[38;2;230;200;100m'       # Natural Gold #E6C864 (13.8:1)
readonly LIME='\033[38;2;160;220;100m'       # Natural Lime #A0DC64 (12.5:1)
readonly SKY_BLUE='\033[38;2;120;190;230m'   # Natural Sky #78BEE6 (10.8:1)
readonly TURQUOISE='\033[38;2;100;200;200m'  # Natural Turquoise #64C8C8 (11.2:1)
readonly VIOLET='\033[38;2;200;140;230m'     # Natural Violet #C88CE6 (8.9:1)
readonly PINK='\033[38;2;230;140;180m'       # Natural Pink #E68CB4 (9.5:1)
readonly LIGHT_GREEN='\033[38;2;140;220;140m' # Natural Light Green #8CDC8C (12.8:1)

# ─── Special Purpose Colors (Eye-friendly with bold) ───
readonly SUCCESS='\033[1;38;2;120;220;120m'  # Success (Bold Soft Green) - Reduced glare
readonly ERROR='\033[1;38;2;255;120;120m'    # Error (Bold Soft Red) - Reduced glare
readonly WARNING='\033[1;38;2;240;160;100m'  # Warning (Bold Natural Orange)
readonly INFO='\033[38;2;120;190;230m'       # Info (Natural Sky)
readonly HIGHLIGHT='\033[1;38;2;230;220;100m' # Highlight (Bold Soft Yellow) - Reduced glare
...
exit_with_cleanup() {
    ...
    if [[ $exit_code -eq 0 ]]; then
        ...
        echo ""
        print_info "Terminal will close automatically in 3 seconds..."
        /bin/sleep 3
        /usr/bin/osascript -e 'tell application "Terminal" to close (every window whose name contains "playcover")' & exit 0
    else
        print_error "$message"
        echo ""
        print_warning "An error occurred. Please check the log."
        echo ""
        echo -n "Press Enter to close terminal..."
        read
...
authenticate_sudo() {
    ...
    if /usr/bin/sudo -v; then
        ...
        exit_with_cleanup 1 "Failed to obtain administrator privileges"
    fi
}

check_playcover_app() {
    if [[ ! -d "/Applications/PlayCover.app" ]]; then
        print_error "PlayCover not found"
        print_warning "Please install PlayCover in /Applications"
        exit_with_cleanup 1 "PlayCover not found"
    fi
}
...
check_full_disk_access() {
    ...
    if /bin/ls "$test_path" >/dev/null 2>&1; then
        ...
        print_warning "Terminal does not have Full Disk Access permission"
        print_info "Please enable it from System Settings > Privacy & Security > Full Disk Access"
        echo ""
        echo -n "After setup, press Enter..."
        read
        ...
        if /bin/ls "$test_path" >/dev/null 2>&1; then
            ...
            if [[ ! "$continue_choice" =~ ^[Yy]$ ]]; then
                exit_with_cleanup 1 "Full Disk Access permission required"
            fi
            echo ""
        fi
    fi
...
acquire_mapping_lock() {
    ...
    while ! /bin/mkdir "$MAPPING_LOCK_FILE" 2>/dev/null; do
        ...
        if [[ $elapsed -ge $((timeout * 10)) ]]; then
            print_error "Failed to acquire mapping file lock (timeout)"
            return 1
        fi
    done
...
check_mapping_file() {
    if [[ ! -f "$MAPPING_FILE" ]]; then
        print_warning "Mapping file not found"
        
        if [[ -x "$INITIAL_SETUP_SCRIPT" ]]; then
            print_info "Please run initial setup script"
            print_info "Run: $INITIAL_SETUP_SCRIPT"
        else
            print_info "Creating empty mapping file"
            touch "$MAPPING_FILE"
        fi
