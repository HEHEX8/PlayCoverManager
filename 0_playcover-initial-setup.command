...
create_playcover_volume() {
    ...
    # Create APFS volume on the container (suppress verbose output)
    if sudo diskutil apfs addVolume "$container" APFS "${VOLUME_NAME}" -nomount > /tmp/apfs_create.log 2>&1; then
...
install_homebrew() {
    ...
    # Install Homebrew (suppress output)
...
install_playcover() {
    ...
    # Install via Homebrew Cask (suppress output)
