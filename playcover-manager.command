...
install_ipa_to_playcover() {
    ...
    if [[ ! -x "$playcover_cli" ]]; then
        print_error "PlayCover CLI が見つかりません"
        return 1
    fi
...
show_quick_status() {
    ...
    if [[ $total_count -gt 0 ]]; then
        ...
        echo "  ${GREEN}🔌 外部ストレージ:${NC} ${external_count}/${total_count}"
        echo "  ${YELLOW}💾 内蔵ストレージ:${NC} ${internal_count}/${total_count}"
        echo "  ${BLUE}⚪ データなし:${NC} ${unmounted_count}/${total_count}"
        echo ""
    fi
}
...
show_menu() {
    ...
    echo ""
    echo "  ${GREEN}【インストール】${NC}"
    echo "  1. IPA をインストール（単一）"
    echo "  2. IPA を一括インストール"
    echo ""
    echo "  ${YELLOW}【ボリューム管理】${NC}"
    echo "  3. 全ボリュームをマウント"
    echo "  4. 全ボリュームをアンマウント"
    echo "  5. 個別ボリューム操作"
    echo ""
    echo "  ${CYAN}【ストレージ管理】${NC}"
    echo "  6. ストレージ切り替え（内蔵⇄外部）"
    echo "  7. ストレージ状態確認"
    echo ""
    echo "  ${RED}【システム】${NC}"
    echo "  8. ディスク全体を取り外し"
